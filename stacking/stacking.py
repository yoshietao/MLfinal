...
for i in range(496465):
	input4.append([NN[i].tolist(),RF[i].tolist(),GTB[i].tolist(),DT[i].tolist()])
input4 = np.array(input4).reshape(496465,20)
...
for i in range(606779):
	test4.append([NN1[i].tolist(),RF1[i].tolist(),GTB1[i].tolist(),DT1[i].tolist()])
test4 = np.array(test4).reshape(606779,20)

x = np.array(model.predict(test4))
print x
print x.shape
...
for i in range(606779):
	answer[i] = np.argmax(x[i])


fout = open('stacking_0.csv','w')
fout.write('id,label\n')
for i in range(606779):
	fout.write(str(i+1)+','+str(int(answer[i]))+'\n')
