...
int main()
{
	...
	for(int i=0;i<rawdata.size();i++)
	{
		...
		while(getline(ss,token,','))
		{
			if(num==1)
			{
				if(strcmp(token.c_str(),"icmp")==0) token=string("0,0,1");
				if(strcmp(token.c_str(),"tcp")==0) token=string("0,1,0");
				if(strcmp(token.c_str(),"udp")==0) token=string("1,0,0");
				newstr=newstr+token+',';
				num++;
			}
