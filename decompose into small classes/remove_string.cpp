...
int main()
{
	...
	for(int i=0;i<rawdata.size();i++)
	{
		...
		while(getline(ss,token,','))
		{
			...
			if(num==41)
			{
				if(strcmp(token.c_str(),"back.")==0) token=string("1");   //1
				if(strcmp(token.c_str(),"buffer_overflow.")==0) token=string("2"); //2
				if(strcmp(token.c_str(),"ftp_write.")==0) token=string("3"); //3
				if(strcmp(token.c_str(),"guess_passwd.")==0) token=string("4"); //3
				if(strcmp(token.c_str(),"imap.")==0) token=string("5");  //3
				if(strcmp(token.c_str(),"ipsweep.")==0) token=string("6"); //4
				if(strcmp(token.c_str(),"land.")==0) token=string("7");  //1
				if(strcmp(token.c_str(),"loadmodule.")==0) token=string("8"); //2
				if(strcmp(token.c_str(),"multihop.")==0) token=string("9");  //3
				if(strcmp(token.c_str(),"neptune.")==0) token=string("10");  //1
				if(strcmp(token.c_str(),"nmap.")==0) token=string("11");  //4
				if(strcmp(token.c_str(),"normal.")==0) token=string("12"); //0
				if(strcmp(token.c_str(),"perl.")==0) token=string("13");  //2
				if(strcmp(token.c_str(),"phf.")==0) token=string("14"); //3
				if(strcmp(token.c_str(),"pod.")==0) token=string("15"); //1
				if(strcmp(token.c_str(),"portsweep.")==0) token=string("16"); //4
				if(strcmp(token.c_str(),"rootkit.")==0) token=string("17");  //2
				if(strcmp(token.c_str(),"satan.")==0) token=string("18");//4
				if(strcmp(token.c_str(),"smurf.")==0) token=string("19");//1
				if(strcmp(token.c_str(),"spy.")==0) token=string("20"); //3
				if(strcmp(token.c_str(),"teardrop.")==0) token=string("21");//1
				if(strcmp(token.c_str(),"warezclient.")==0) token=string("22");//3
				if(strcmp(token.c_str(),"warezmaster.")==0) token=string("0");//3
			}
			...
			if(num==41)
			{
				...
				//ofs<<token<<endl;
				rawdata[i] = newstr.substr(0,newstr.size()-1);
				id[i] = token;
				for(int i=0;i<23;i++)
				{
					string ii;
 	...
 	for(int i=0;i<23;i++) cout<<i<<' '<<count[i]<<endl;
 		
 	for(int i=0;i<m.size();i++)
 	{
 		//ofs<<rawdata[i]<<id[i]<<endl;
 		ofs<<rawdata[i]<<endl;
 		ofs<<id[i]<<endl;
 	}
/*
